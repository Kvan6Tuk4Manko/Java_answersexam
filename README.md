...
### Различия между статическими и нестатическими членами класса
...
Концепция неизменяемых классов в объектно-ориентированном программировании (ООП) подразумевает создание таких классов, экземпляры которых не могут быть изменены после их создания. Это означает, что после инициализации объекта его состояние остается неизменным на протяжении всего времени его существования.

### Что делает класс неизменяемым?

Чтобы класс был неизменяемым, необходимо выполнить несколько условий:

1. **Отсутствие открытых полей для записи**: Все поля должны быть приватными (private), чтобы внешний код не мог напрямую изменять их значения.
2. **Использование `final` для класса**: Если язык программирования поддерживает это ключевое слово (например, Java), можно пометить сам класс как `final`, чтобы предотвратить наследование и возможность изменения состояния через дочерние классы.
3. **Использование `final` для полей**: Все поля должны быть помечены как `final`, чтобы гарантировать, что они будут инициализированы только один раз при создании объекта и больше не изменяются.
4. **Отсутствие методов для изменения состояния**: Никакие методы класса не должны изменять состояние объекта после его создания.
5. **Глубокая копия для изменяемых объектов**: Если класс содержит ссылки на другие изменяемые объекты, эти объекты также должны быть защищены от изменения. Обычно это достигается с помощью глубокой копии (deep copy).

### Пример создания неизменяемого класса на языке Java
...
public final class ImmutablePoint {
    ...
    // Методы для работы с объектом, но не изменяющие его состояние
...
### Объяснение примера:
1. **Класс помечен как `final`**, что предотвращает его наследование и возможность создания подклассов, которые могли бы изменять поведение или состояние объекта.
2. **Поля `x` и `y` объявлены как `private final`**, что гарантирует, что они будут инициализированы только один раз в конструкторе и не будут изменяться после этого.
3. **Метод `move` возвращает новый объект** вместо изменения текущего. Это позволяет сохранять неизменность исходного объекта.
...
public class Main {
    public static void main(String[] args) {
        ...
        ImmutablePoint movedPoint = point.move(5, 5);
        System.out.println("Moved Point: (" + movedPoint.getX() + ", " + movedPoint.getY() + ")");
        System.out.println("Original Point after move: (" + point.getX() + ", " + point.getY() + ")");
    }
}
```
...
Moved Point: (15, 25)
Original Point after move: (10, 20)
```

Как видно из примера, исходный объект `point` остался неизменным, а новый объект `movedPoint` был создан для представления нового состояния.
...
В объектно-ориентированном программировании (ООП) создание объектов является одной из базовых операций. Существуют различные способы создания объектов, и каждый из них имеет свои особенности и области применения.

#### Стандартный способ создания объекта с использованием `new`

Самый прямой и простой способ создания объекта — это использование ключевого слова `new`. Этот метод используется для вызова конструктора класса и создания нового экземпляра объекта.
...
Здесь создается новый объект типа `Car` с указанными маркой и моделью.

### Фабричные методы

Фабричные методы — это альтернативный способ создания объектов, который позволяет скрыть детали создания объекта от клиента и делегировать эту задачу специальным методам. Фабричные методы часто используются в паттернах проектирования, таких как **Factory Method** и **Abstract Factory**.

Основные преимущества фабричных методов:
1. **Абстракция**: Клиентский код не зависит от конкретных типов создаваемых объектов.
2. **Гибкость**: Легко добавлять новые типы объектов без изменения существующего кода.
3. **Инкапсуляция**: Детали создания объекта инкапсулированы внутри фабрики.

### Отличие фабричных методов от стандартного создания объектов с использованием `new`

1. **Абстракция**: В случае использования `new`, клиент должен знать конкретный тип объекта, который он хочет создать. Фабричные методы позволяют абстрагироваться от конкретного типа объекта и работать только с интерфейсами или абстрактными классами.
2. **Гибкость**: С фабричными методами легче изменять логику создания объектов без изменения клиентского кода. Это особенно полезно, если требуется поддержка нескольких реализаций одного интерфейса.
3. **Управление зависимостями**: Фабричные методы могут управлять зависимостями между объектами, что может упростить управление сложностью системы.

### Примеры использования фабричных методов

#### Пример 1: Simple Factory Pattern

Рассмотрим пример создания объектов автомобилей с использованием простой фабрики.
...
// Простая фабрика для создания автомобилей
class CarFactory {
...
Теперь рассмотрим более сложный пример с использованием паттерна Factory Method.
...
// Конкретная фабрика для создания Toyota
...
// Конкретная фабрика для создания Honda
...
В объектно-ориентированном программировании (ООП) отношения между классами играют ключевую роль в определении структуры и поведения системы. Различные виды отношений позволяют моделировать взаимодействие объектов и их зависимости друг от друга. Рассмотрим основные виды отношений между классами: **ассоциация**, **агрегация**, **композиция** и **наследование**.

### 1. Ассоциация (Association)

**Ассоциация** — это общее понятие, описывающее связь между двумя или более классами. Она указывает на то, что один класс использует другой, но не обладает им. Ассоциация может быть односторонней или двусторонней.
...
Здесь `Teacher` имеет метод `teach`, который принимает объект `Student`. Это пример ассоциации, так как `Teacher` использует `Student`, но не владеет им.

### 2. Агрегация (Aggregation)

**Агрегация** — это частный случай ассоциации, при котором один класс содержит ссылки на другие классы, но эти объекты могут существовать независимо от содержащего класса. Агрегация представляет собой "has-a" (имеет) отношение, где содержащий класс не является владельцем объектов, которые он содержит.
...
Здесь `Department` содержит список студентов (`List<Student>`). Однако студенты могут существовать независимо от отделения, поэтому это пример агрегации.

### 3. Композиция (Composition)

**Композиция** — это более строгая форма агрегации, при которой содержащий класс полностью владеет объектами, которые он содержит. Если содержащий объект уничтожается, то все его составные части также уничтожаются. Композиция также представляет собой "has-a" (имеет) отношение, но с более сильной зависимостью.
...
class Car {
    ...
    public Car() {
        this.engine = new Engine(); // Создание экземпляра Engine внутри Car
    }
...
Здесь `Car` содержит объект `Engine`, который создается вместе с объектом `Car` и существует только пока существует сам `Car`. Это пример композиции.

### 4. Наследование (Inheritance)

**Наследование** — это механизм, который позволяет одному классу (дочернему) наследовать свойства и методы другого класса (родительского). Наследование используется для создания иерархий классов и поддерживает принцип DRY (Don't Repeat Yourself).
...
public class Main {
    public static void main(String[] args) {
        Dog dog = new Dog();
        dog.eat();  // Метод наследуется от Animal
        dog.bark(); // Метод, специфичный для Dog
    }
}
```

Здесь класс `Dog` наследует от класса `Animal`, получая доступ к его методам и свойствам. Это пример наследования.

### Сравнение отношений

| Вид отношения | Описание | Пример |
|---------------|----------|--------|
| **Ассоциация** | Общий тип связи, когда один класс использует другой. | Учитель учит студента. |
| **Агрегация** | Связь "has-a", где объекты могут существовать независимо. | Отделение содержит студентов. |
| **Композиция** | Строгая форма агрегации, где объекты не могут существовать без владельца. | Машина содержит двигатель. |
| **Наследование** | Класс наследует свойства и методы другого класса. | Собака наследует от животного. |
...
В Java и некоторых других языках программирования, примитивные типы данных (например, `int`, `boolean`, `char` и т.д.) не являются объектами. Однако в некоторых ситуациях требуется работать с примитивными типами как с объектами, например, при использовании коллекций или передаче параметров методам, которые принимают объекты.

Для решения этой задачи в Java используются **классы-обертки** (wrapper classes). Каждый примитивный тип имеет соответствующий класс-обертку, который предоставляет функциональность для работы с этим типом как с объектом.
...
### Преобразование примитивных типов в объекты и обратно

#### 1. Преобразование примитивного типа в объект

Для преобразования примитивного типа в объект используется конструктор соответствующего класса-обертки.

Пример:

```java
int primitiveInt = 42;
Integer integerObject = new Integer(primitiveInt); // Устаревший способ

// Современный способ с использованием метода valueOf
Integer integerObjectModern = Integer.valueOf(primitiveInt);
```

Современные версии Java рекомендуют использовать статический метод `valueOf`, так как он может использовать кэширование для оптимизации памяти.

#### 2. Преобразование объекта в примитивный тип

Для преобразования объекта в примитивный тип можно использовать методы соответствующего класса-обертки, такие как `intValue()`, `doubleValue()` и т.д.
...
#### 3. Автоупаковка (Autoboxing) и автовыделение (Unboxing)

Java автоматически выполняет упаковку (boxing) и выделение (unboxing) примитивных типов в объекты и обратно, что значительно упрощает работу с классами-обертками.

Пример автоупаковки:

```java
Integer integerObject = 42; // Автоматическая упаковка примитивного int в Integer
```

Пример автовыделения:

```java
int primitiveInt = integerObject; // Автоматическое выделение Integer в int
```
...
Коллекции в Java могут хранить только объекты, поэтому для хранения примитивных типов необходимо использовать их обертки.

Пример:

```java
List<Integer> integerList = new ArrayList<>();
integerList.add(10); // Автоматическая упаковка int в Integer
integerList.add(20);

for (Integer num : integerList) {
    System.out.println(num); // Автоматическое выделение Integer в int при необходимости
}
```

#### 2. Передача параметров методам

Методы, принимающие объекты, могут работать с классами-обертками.
...
public class WrapperExample {
    ...
    public static void main(String[] args) {
        int primitiveInt = 5;
        printValue(primitiveInt); // Автоматическая упаковка int в Integer
    }
}
```
...
Классы-обертки предоставляют методы для сравнения значений.
...
System.out.println(a.equals(b)); // true, так как значения равны

Integer c = 1000;
Integer d = 1000;

System.out.println(c == d); // false, так как это разные объекты (не кэшируются)
```
...
В Java класс `String` представляет собой последовательность символов (строку) и является одним из самых часто используемых классов. Одной из ключевых особенностей класса `String` является его **неизменяемость** (иммутабельность), что означает, что после создания объекта строки его содержимое не может быть изменено.

### Понятие неизменяемости строк в Java

**Неизменяемость** (`immutability`) строки означает, что после создания объекта строки его состояние не может быть изменено. Любые операции, которые кажутся изменяющими строку, на самом деле создают новый объект строки с обновленным содержимым, а исходный объект остается неизменным.

Преимущества неизменяемости:
1. **Безопасность потоков**: Поскольку строки неизменяемы, их можно безопасно использовать в многопоточных приложениях без необходимости синхронизации.
2. **Кэширование хеш-кодов**: Хеш-код строки вычисляется один раз при создании объекта и затем кэшируется, что ускоряет выполнение операций, таких как сравнение строк и использование их в коллекциях, таких как `HashSet` или `HashMap`.
3. **Повторное использование**: Неизменяемые строки могут быть легко кэшированы и повторно использованы, что экономит память.

### Создание объектов типа `String`

Создание объектов типа `String` можно выполнять несколькими способами:

#### 1. Использование литерала строки
```java
String str1 = "Hello";
```
Этот способ создает строку в специальной области памяти, называемой **пулом строк** (string pool). Если такая строка уже существует в пуле, будет использована ссылка на существующий объект.

#### 2. Использование конструктора
```java
String str2 = new String("Hello");
```
Этот способ всегда создает новый объект строки, даже если такая строка уже существует в пуле.

#### 3. Создание строки из массива символов
```java
char[] chars = {'H', 'e', 'l', 'l', 'o'};
String str3 = new String(chars);
```

### Примеры работы с методами создания, сравнения и модификации строк
...
System.out.println(str1 == str2); // false, так как str2 создан через new и находится в куче
...
System.out.println(str1.equals(str2)); // true, так как содержимое строк одинаковое
System.out.println(str1.equals(str3)); // false, так как содержимое строк разное

// Сравнение с учетом регистра
System.out.println(str1.equalsIgnoreCase("hello")); // true

// Сравнение по порядку лексикографически
System.out.println(str1.compareTo(str2)); // 0, строки равны
System.out.println(str1.compareTo(str3)); // отрицательное число, так как "Hello" < "World"
```

#### Модификация строк
Хотя строки неизменяемы, существуют методы для создания новых строк на основе старых.

```java
String str = "Hello";

// Конкатенация строк
String result1 = str.concat(" World"); // "Hello World"

// Объединение строк с использованием +
String result2 = str + " World"; // "Hello World"
...
// Замена символов
String replacedStr = str.replace('H', 'J'); // "Jello"
```
